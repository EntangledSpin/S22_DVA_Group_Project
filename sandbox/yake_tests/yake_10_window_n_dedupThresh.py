...
experiment_name = "YAKE - windowsSize, n,dedupThreshold "
experiment_parameter = 'windowsSize, n, dedupThreshold'
...
notes = """Iterating through window size [1-5], n [1-3] , dedup thresh [.1-.9]"""
...
param_list = [1, 2, 3, 4, 5] # windows size
param_list_1 = [1, 2, 3] # max n
param_list_2 = [.1, .2, .3, .4, .5, .6, .7, .8, .9] # dedup threshold

perms = [(x,y,z) for x in param_list for y in param_list_1 for z in param_list_2]
...
for perm in perms:
    ...
    ## ensure i is assigned to expiremental parameter -
    # with defaults set to itself -  values are defined above in experiment parameters section

    language = language
    max_ngram_size = perm[1]
    deduplication_threshold = perm[2]
